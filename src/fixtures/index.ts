...
async function createSession(
  ...
  if (fs.existsSync(filePath)) {
    logger.debug(
      `Auth: loading cached session from ${browserName}/${storageFile}`,
    );
  ...
  logger.warn(
    `Auth: no cached state for "${credentials.username}" on ${browserName} — performing fresh login`,
  );
...
export const test = base.extend<Fixtures>({
  standard: async ({ browser, browserName }, use) => {
    const pages = await createSession(
      browser,
      browserName,
      "standard_user.json",
      users.standard,
