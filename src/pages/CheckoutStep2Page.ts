...
export class CheckoutStep2Page extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.items = page.locator('[data-test="checkout-summary-container"] .cart_item');
  ...
  async finish(): Promise<void> {
    this.page.on('response', (response: Response) => {
      if (response.url().includes('submit.backtrace.io') && response.status() === 503) {
        logger.warn('CheckoutStep2: error telemetry returned 503 — fault injection active (error_user)');
      }
    });
