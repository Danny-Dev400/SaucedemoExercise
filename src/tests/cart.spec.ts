...
test.describe('Suite 3 — Cart', () => {
  ...
  test('TC-18 — Add a single item to cart — badge updates to 1', async ({ standard: pages }) => {
  ...
  test('TC-19 — Add multiple items — badge count updates correctly', async ({ standard: pages }) => {
  ...
  test('TC-20 — Remove item from inventory page', async ({ standard: pages }) => {
    await pages.inventoryPage.addToCart(PRODUCT);
    await expect(pages.inventoryPage.header.cartBadge).toHaveText('1');
    await pages.inventoryPage.removeFromCart(PRODUCT);
    await expect(pages.inventoryPage.header.cartBadge).toBeHidden();
    await expect(pages.page.locator(`[data-test="add-to-cart-${PRODUCT}"]`)).toBeVisible();
  });

  test('TC-21 — Cart page displays added items correctly', async ({ standard: pages }) => {
    await pages.inventoryPage.addToCart(products[0].slug);
    await pages.inventoryPage.addToCart(products[1].slug);
    ...
    await expect(pages.cartPage.itemNames).toHaveText([products[0].name, products[1].name]);
    await expect(pages.cartPage.itemPrices).toHaveText([products[0].price, products[1].price]);
  ...
  test('TC-24 — Continue shopping returns to inventory', async ({ standard: pages }) => {
    await pages.inventoryPage.addToCart(PRODUCT);
    await pages.inventoryPage.header.openCart();
