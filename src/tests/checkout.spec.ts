...
test.describe('Suite 4 — Checkout', () => {
  ...
  test('TC-26 — Checkout step 1 — required field validations', async ({ standard: pages }) => {
    await goToCheckout(pages);

    await pages.checkoutStep1.fill({ ...checkoutForms.standard, firstName: '' });
    await pages.checkoutStep1.continue();
    await expect(pages.checkoutStep1.errorBanner).toContainText('Error: First Name is required');
    ...
    await pages.checkoutStep1.continue();
    await expect(pages.checkoutStep1.errorBanner).toContainText('Error: Last Name is required');

    await pages.checkoutStep1.dismissError();
    await pages.checkoutStep1.clearAll();
    await pages.checkoutStep1.fill({ ...checkoutForms.standard, postalCode: '' });
    await pages.checkoutStep1.continue();
    await expect(pages.checkoutStep1.errorBanner).toContainText('Error: Postal Code is required');
  });

  test('TC-27 — Checkout step 2 — order summary price breakdown', async ({ standard: pages }) => {
    await goToCheckout(pages);
    await pages.checkoutStep1.fill(checkoutForms.standard);
    await pages.checkoutStep1.continue();
    await expect(pages.page).toHaveURL(/\/checkout-step-two\.html/);
  ...
  test('TC-28 — Cancel checkout from step 1 returns to cart', async ({ standard: pages }) => {
    await goToCheckout(pages);
    await expect(pages.page).toHaveURL(/\/checkout-step-one\.html/);
  ...
  test('TC-29 — Cancel checkout from step 2 returns to inventory', async ({ standard: pages }) => {
    await goToCheckout(pages);
    await pages.checkoutStep1.fill(checkoutForms.standard);
    await pages.checkoutStep1.continue();
    await expect(pages.page).toHaveURL(/\/checkout-step-two\.html/);
  ...
  test('TC-30 — Error User — Intermittent Checkout Form Errors - Fail on Purpose', async ({ error: pages }) => {
    await goToCheckout(pages);
    await expect(pages.page).toHaveURL(/\/checkout-step-one\.html/);
  ...
  test('TC-34 — Checkout with multiple products — subtotal matches sum of item prices', async ({ standard: pages }) => {
    ...
    await expect(pages.inventoryPage.header.cartBadge).toHaveText(`${selectedProducts.length}`);
    await pages.inventoryPage.header.openCart();
    ...
    await expect(pages.checkoutStep2.items).toHaveCount(selectedProducts.length);
    const { subtotal, tax, total } = await pages.checkoutStep2.getPrices();
