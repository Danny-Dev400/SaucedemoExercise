...
test.describe('Suite 2 — Product Catalogue', () => {
  for (const { label, userKey } of visualCases) {
    test(`TC-09 — Inventory page displays product grid correctly — ${label}`, async ({ browser, browserName }) => {
      ...
      try {
        ...
        await expect(pages.inventoryPage.activeSort).toHaveText('Name (A to Z)');
  ...
  test('TC-10 — Sort products by name A to Z (default)', async ({ standard: pages }) => {
    await pages.inventoryPage.goto();
  ...
  test('TC-12 — Sort products by price low to high', async ({ standard: pages }) => {
    await pages.inventoryPage.goto();
  ...
  test('TC-13 — Sort products by price high to low', async ({ standard: pages }) => {
    await pages.inventoryPage.goto();
  ...
  test('TC-14 — Navigate to product detail page and back', async ({ standard: pages }) => {
    ...
    await expect(pages.productDetail.description).toHaveText(firstProduct.description);
  ...
  test('TC-15 — Add to cart from product detail page', async ({ standard: pages }) => {
    const firstProduct = products[0];
    await pages.inventoryPage.goto();
    await pages.inventoryPage.openFirstProduct();
    await expect(pages.productDetail.name).toHaveText(firstProduct.name);
    ...
    await expect(pages.inventoryPage.removeButton(firstProduct.slug)).toBeVisible();
  });
});
